...
public class FabricCursorMod implements ClientModInitializer {
	...
	private void renderScreen(Screen gui, DrawContext context, int mouseX, int mouseY, float tickDelta) {
		...
		bufferSource.getBuffer(FabricRenderLayers.CURSOR);
		...
		if (mod.getConfig().dynamicCursor) {
							...
							if (o instanceof TextFieldWidget) {
								...
								for (Object e : (Iterable<?>) o)
									if (e instanceof PressableWidget b) {
										if (isHoverButton(mouseX, mouseY, b))
											newCursorType = CursorType.HAND;
									} else if (e instanceof TextFieldWidget b) {
			...
			if (gui instanceof HandledScreen<?> container) {
				ClientPlayerEntity player = ((ScreenAccessor) gui).getClient().player;
				if (player != null && !player.currentScreenHandler.getCursorStack().getItem().equals(Items.AIR))
					newCursorType = CursorType.HAND_GRAB;
				else if (((HandledScreenAccessor) container).getFocusedSlot() != null && ((HandledScreenAccessor) container).getFocusedSlot().hasStack())
					newCursorType = CursorType.HAND;
			} else if (gui instanceof ChatScreen) {
				MinecraftClient mc = ((ScreenAccessor) gui).getClient();
				int mx = (int) (mc.mouse.getX() * (double) mc.getWindow().getScaledWidth()
						/ (double) mc.getWindow().getFramebufferWidth());
				int my = (int) (mc.mouse.getY() * (double) mc.getWindow().getScaledHeight()
						/ (double) mc.getWindow().getFramebufferHeight());
