...
public class GuiCursorConfig extends ScreenListener {
	...
	public void init() {
		...
		((ForgeCommonTextField)xhotspot).handle.setFocused(false);
		((ForgeCommonTextField)yhotspot).handle.setFocused(false);
		((ForgeCommonTextField)cursorLocation).handle.setFocused(false);
		((ForgeCommonTextField)cursorSize).handle.setFocused(false);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
		...
		if (xhotspot.mouseClicked(mouseX, mouseY, mouseButton)) {
			((ForgeCommonTextField)xhotspot).handle.setFocused(true);
			handled = true;
		}
		if (yhotspot.mouseClicked(mouseX, mouseY, mouseButton)) {
			((ForgeCommonTextField)yhotspot).handle.setFocused(true);
			handled = true;
		}
		if (cursorLocation.mouseClicked(mouseX, mouseY, mouseButton)) {
			((ForgeCommonTextField)cursorLocation).handle.setFocused(true);
			handled = true;
		}
		if (cursorSize.mouseClicked(mouseX, mouseY, mouseButton)) {
			((ForgeCommonTextField)cursorSize).handle.setFocused(true);
			handled = true;
		}
